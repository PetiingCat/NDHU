...
var
  Form1: TForm1; path,path2:string;
...
var s,s1,s2,s3,fname,msg:string; i,k,f,cl:integer; ms:TMemorystream;
begin
    cl:=-1;
    ...
    {HTTP Message Judgement}
      ...
      begin
        {!--MonitorClient--}listbox2.Items.Add('['+DatetimeToStr(now())+'] Send Variable:'+msg);
        msg:='Exception';
      end;
      if msg='/' then
      begin{!--MonitorClient--}listbox2.Items.Add('['+DatetimeToStr(now())+'] Web-connected');end;
    ...
    {!--viewEvent--}listbox1.Items.Add('{---Server_FileSend/User_DownloadFile---}');
      ...
      begin f:=1;ms.LoadFromFile(path2+'HTML\webicon.png');end//If bowser require web's icon
      else
      begin
        i:=0;
        repeat
        ...
        until i=FLB.Count;
        if f=1 then
        begin ms.LoadFromFile(path+fname);end
        else msg:='Exception';
      end;
      if f=1 then
      begin
      ...
      ms.Free;
      {!--MonitorClient--}listbox2.Items.Add('['+DatetimeToStr(now())+'] Get file: '+fname);
      end;
    end;
    ...
    {!--viewEvent--}listbox1.Items.Add('{---Server_FileReceive&Save/User_UploadFile---}');
        ...
        ms.Free;
        {!--MonitorClient--}listbox2.Items.Add('['+DatetimeToStr(now())+'] Upload file: '+fname);
    end;
    ...
    ms:=Tmemorystream.Create;
    ms.LoadFromFile(path2+'HTML\page0403.html');
    setlength(s,ms.Size);
    ms.Read(s[1],ms.Size);
    ms.Free;
    f:=pos('/*%Echo%*/',s);
    ...
    begin
       s1:=copy(s,1,f-1);
       s2:=copy(s,f+10,length(s));
       for i:=0 to FLB.Count-1 do
       begin
          s3:=s3+'str+="<tr><th>'+FLB.Items[i]+'</th><th><a href=\"http://127.0.0.1:7000/'+FLB.Items[i]+'\">Download</a></th></tr>";';
       end;
       s:=s1+s3+s2;
    end;
...
begin
     {SetDataBasePath}
     path:='D:\大四上\企業網路\EP_EXProject\OldVerson\verson4\FileFolder\';
     FLB.Directory:=path;
     path2:='D:\大四上\企業網路\EP_EXProject\OldVerson\verson4\';
end;
