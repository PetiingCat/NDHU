...
#	Reference: this sode is edit from https://github.com/hakjong (2012105056 Hakjong Son)
#	Change: adding console input and ouput
#	Describe how this sorting algorithm work?
#		1. Picking a pivot in the array(in this code, I initialize piovt at the first one in every array)
#		2. Use loop to reapeat the job of "choosing a new pivot" and "adjusting the order of array(Partition)".
#		3. Procedure "Partition" is using swap to turn array into three parts like:
#									array(smaller than pivot), pivot(final position), array(greater than pivot)
#		4. Time Complexity: Ο(n log n)~Ο(n2)  Space Complexity: Ο(log n) ~ Ο(n)
#
